...
import { useTenants, type UpdateTenantRequest } from "@/hooks/use-tenants";

// Use the same schema as the hook for consistency
...
function GeneralPage() {
  ...
  const tenant = getTenant(currentTenant?.id || "");
  ...
  // Update form when tenant data loads - use tenant from getTenant
  React.useEffect(() => {
    if (tenant) {
      form.reset({
        name: tenant.name,
